...
export default function StockManager({ item, onClose, onChanged }) {
  ...
  const inputClass =
    'w-24 border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand';
    ...
    <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-bold text-brand">Manage Stock</h2>
                ...
                <div key={key} className="border border-gray-200 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-lg font-bold text-brand">
                      {qtyFor(key)} <span className="text-sm font-normal text-gray-400">{item.unit}</span>
                    </span>
                  </div>
